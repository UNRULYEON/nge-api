...
- The OpenAPI/Scalar docs are served at the root path `/` (not `/openapi`); the raw spec is at `/openapi.json`. `GET /` returns `src/modules/docs/llms.md` when `Accept` prefers `text/markdown`, or when Accept is unspecified and the User-Agent looks like an AI agent. Explicit `Accept: text/html` still gets Scalar. The guide is always at `/llms.txt`. Health is at `/health`. All data routes are versioned under `/v1` (e.g. `/v1/studios`, `/v1/shows`, `/v1/movies`, `/v1/shows/:id/episodes`, `/v1/studios/:id/movies`). Agents can also use Streamable HTTP MCP at `/v1/mcp` (`src/modules/mcp`). Production healthchecks use `/health` via `railway.json`.
